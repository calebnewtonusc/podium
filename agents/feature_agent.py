...
class FeatureAgent:
    ...
    def engineer(
        ...
        feature_generators = [
            (
                "group_stats",
                lambda: self._group_statistics(
                    train, test, categorical_cols, numeric_cols[:10], target
                ),
            ),
            (
                "ratio_features",
                lambda: self._ratio_features(train, test, numeric_cols[:15]),
            ),
            (
                "polynomial",
                lambda: self._polynomial_features(
                    train, test, eda_results.get("top_correlated_features", {})
                ),
            ),
            (
                "datetime_features",
                lambda: self._datetime_features(train, test, datetime_cols),
            ),
            (
                "target_encoding",
                lambda: self._target_encoding(train, test, high_card_cols, target),
            ),
            (
                "frequency_encoding",
                lambda: self._frequency_encoding(train, test, categorical_cols),
            ),
            (
                "lag_features",
                lambda: self._lag_features(
                    train, test, datetime_cols, numeric_cols[:5], target_column
                ),
            ),
        ]
        ...
        for gen_name, gen_fn in feature_generators:
            ...
            if improvement > 0.0001:
                ...
                logger.info(
                    f"{gen_name}: +{improvement:.5f} → {best_score:.5f} ({len(new_features)} new features)"
                )
            else:
                logger.debug(
                    f"{gen_name}: no improvement ({trial_score:.5f} vs {best_score:.5f})"
                )

        logger.info(
            f"Feature engineering complete. CV: {baseline_score:.5f} → {best_score:.5f}"
        )
        logger.info(
            f"Features: {len(feature_cols)} → {len(best_feature_cols)} (+{len(all_generated)} generated)"
        )
    ...
    def _quick_cv(
        ...
        for train_idx, val_idx in cv.split(X, y):
            ...
            if is_classification:
                ...
                scores.append(
                    metric_fn(y.iloc[val_idx], model.predict(X.iloc[val_idx]))
                )
    ...
    def _lag_features(
        ...
        if use_test:
            combined = pd.concat(
                [train.assign(__podium_split__=0), test.assign(__podium_split__=1)]
        ...
        for col in value_cols[:3]:
            ...
            for window in [7, 30]:
                ...
                combined[feat_name] = (
                    combined[col].rolling(window, min_periods=1).mean()
                )
                new_features.append(feat_name)
        ...
        train_rows = (
            combined[combined["__podium_split__"] == 0] if use_test else combined
        ).sort_values(date_col)
        ...
        if use_test:
            test_rows = combined[combined["__podium_split__"] == 1].sort_values(
                date_col
            )
