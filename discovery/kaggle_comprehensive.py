...
PYTHON_INDICATORS = [
    'import', 'def ', 'class ', 'pd.read_csv', 'np.', 'sklearn',
    'torch', 'tensorflow', 'keras', 'xgboost', 'lgbm', 'catboost',
    'train_test_split', 'cross_val_score', 'GridSearchCV', 'Pipeline',
]
...
def classify_competition_type(comp_info: dict) -> str:
    ...
    if any(kw in combined for kw in ["image", "vision", "object detection", "segmentation",
                                      "classification image", "cnn", "resnet", "efficientnet"]):
        return "cv"
    if any(kw in combined for kw in ["nlp", "text", "natural language", "sentiment",
                                      "bert", "transformer", "language model", "speech"]):
        return "nlp"
    if any(kw in combined for kw in ["time series", "forecasting", "temporal", "sequence",
                                      "lstm", "arima", "sales forecast"]):
        return "time_series"
    if any(kw in combined for kw in ["tabular", "structured", "csv", "classification",
                                      "regression", "gradient boosting", "xgboost"]):
        return "tabular"
    if any(kw in combined for kw in ["multimodal", "multi-modal", "image and text",
                                      "video", "audio", "3d"]):
        return "multimodal"
...
def download_notebook_content(api, kernel_ref: str, output_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        files = list(output_dir.glob(f"*.ipynb")) + list(output_dir.glob(f"*.py"))
        if files:
...
def fetch_zindi_competitions() -> list[dict]:
    ...
    try:
        ...
        if HAS_HTTPX:
            with httpx.Client(timeout=20) as client:
                resp = client.get(url, headers={"User-Agent": "nalana-data-harvester/1.0"})
                if resp.status_code == 200:
                    data = resp.json()
    ...
    if filepath.exists():
        with open(filepath) as f:
            for line in f:
                try:
                    rec = json.loads(line)
                    seen.add(rec.get("slug") or rec.get("ref") or rec.get("competition", ""))
...
def main():
    ...
    parser.add_argument("--kaggle", action="store_true", help="Kaggle notebooks and competitions")
    parser.add_argument("--discussions", action="store_true",
                        help="Collect Kaggle discussion posts with code")
    parser.add_argument("--external", action="store_true",
                        help="Collect from DrivenData and Zindi")
    parser.add_argument("--min-votes", type=int, default=20,
                        help="Minimum votes for notebooks")
    parser.add_argument("--min-discussion-votes", type=int, default=3,
                        help="Minimum votes for discussion posts")
    parser.add_argument("--max-comps", type=int, default=2000,
    ...
    for i, comp in enumerate(competitions[:args.max_comps]):
        ...
        if args.discussions and slug not in seen_discussions:
            discussions = fetch_competition_discussions(api, slug, args.min_discussion_votes)
